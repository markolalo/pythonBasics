"""Blackjack
The classic card game also known as 21. (This vesrion doesn't have 
...
def main():
    ...
    Rules:
        ...
        The dealer stopes hitting at 17.''')
...
def getDeck():
    """Returns a lost of (rank, suit) tuples for all 52 cards"""
    deck = []
...
def displayHands(playerHand, dealerHand, showDealerHand):
    """Show the player's and dealer's cards. Hide the dealer's first
     card if the showDealer Hand is False"""
    print()
    if showDealerHand:
...
def displayCards(cards):
    ...
    for i, card in enumerate(cards):
        rows[0] += '_____' # Print the top line of the card.
        if card == BACKSIDE:
            # Print a card's back
            rows[1] += '|## | '
            rows[2] += '|###| '
